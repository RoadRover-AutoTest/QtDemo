...
void Model_PicCompare::Cameracompare(QString comPic ,QString fixedPic)
{
    ...
    if (iDiffNum <= 5)
        ShowList.append("校验结果：非常相似");//QMessageBox::information(this,tr("比较结果"),tr("two images are very similar!"));//+"iDiffNum = "+QString("%1").arg(iDiffNum)
    else if (iDiffNum > 10)
        ShowList.append("校验结果：完全不同的图片");//QMessageBox::information(this,tr("比较结果"),tr("they are two different images!"));//+"iDiffNum = "+QString("%1").arg(iDiffNum)
    else
        ShowList.append("校验结果：有些相似");//QMessageBox::information(this,tr("比较结果"),tr("two image are somewhat similar!"));//+"iDiffNum = "+QString("%1").arg(iDiffNum)
}
